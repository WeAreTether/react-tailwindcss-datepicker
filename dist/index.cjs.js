...
const Arrow = React__default["default"].forwardRef((props, ref) => {
    return (React__default["default"].createElement("div", { ref: ref, className: "hidden sm:block absolute z-20 h-4 w-4 rotate-45 mt-0.5 ml-[1.2rem] border-l border-t border-gray-300 bg-white dark:bg-neutral-800 dark:border-neutral-600" }));
});
...
const Days = ({ calendarData, onClickPreviousDays, onClickDay, onClickNextDays }) => {
    ...
    return (React__default["default"].createElement("div", { className: "grid grid-cols-7 gap-y-0.5 my-1" },
        ...
        calendarData.days.next.map((item, index) => (React__default["default"].createElement("button", { type: "button", key: index, disabled: isDateDisabled(index, "next"), className: "flex items-center justify-center text-gray-400 h-12 w-12 lg:w-10 lg:h-10", onClick: () => handleClickDay(item, "next"), onMouseOver: () => {
                hoverDay(item, "next");
            } }, item)))));
};
...
require$$0__default["default"].extend(advancedFormat);
const Input = (e) => {
    // Context
...
const Shortcuts = () => {
    ...
    const shortcutOptions = React.useMemo(() => configs
...
const Datepicker = ({ primaryColor = "blue", value = null, onChange, useRange = true, showFooter = false, showShortcuts = false, configs = null, asSingle = false, placeholder = null, separator = "~", startFrom = null, i18n = LANGUAGE, disabled = false, inputClassName = null, containerClassName = null, toggleClassName = null, toggleIcon = undefined, displayFormat = DATE_FORMAT, readOnly = false, minDate = null, maxDate = null, disabledDates = null, inputId, inputName, startWeekOn = "sun", classNames = undefined }) => {
    ...
    return (React__default["default"].createElement(DatepickerContext.Provider, { value: contextValues },
        React__default["default"].createElement("div", { className: `relative w-full text-gray-700 ${containerClassName}`, ref: containerRef },
            ...
            React__default["default"].createElement("div", { className: "transition-all ease-out duration-300 z-50 mt-[1px] text-sm lg:text-xs 2xl:text-sm translate-y-4 opacity-0 hidden fixed bottom-0 left-0 right-0 m-0 sm:absolute sm:bottom-auto sm:left-auto sm:right-auto", ref: calendarContainerRef },
                React__default["default"].createElement(Arrow, { ref: arrowRef }),
                React__default["default"].createElement("div", { className: "mt-2.5 shadow-sm border border-gray-300 px-1 py-0.5 bg-white dark:bg-neutral-800 dark:text-white dark:border-neutral-600 rounded-lg" },
